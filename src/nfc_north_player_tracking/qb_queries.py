...
def find_team_quarterbacks(csv_path, teams):
    """Return roster-position QBs who have at least one passer play for the teams."""
    return (
        pl.read_csv(
            csv_path,
            columns=[
        ...
        .with_columns(
            ...
            _clean_text(OFFENSE_TEAM).alias(OFFENSE_TEAM),
            _clean_text(OFFENSE_POSITION).alias(OFFENSE_POSITION),
        )
        .filter(
            pl.col(OFFENSE_TEAM).is_in(teams)
            & (pl.col(OFFENSE_POSITION) == "QB")
            & _is_yes(OFFENSE_PASSER_FLAG)
            & (pl.col(OFFENSE_PLAYER_ID) != "")
        )
        .select(
            OFFENSE_TEAM,
            OFFENSE_PLAYER_NAME,
        ...
        .unique()
        .sort([OFFENSE_TEAM, OFFENSE_PLAYER_NAME])
    )
